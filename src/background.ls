...
require! 'prelude-ls': { map, join, each, filter, is-type, empty, last }
...
create-notification-options = (task, data) ->
  console.count 'create-notification-options'
  options =
    type: 'basic'
...
persistor = persist-store redux-store, configure-sync!, ->
  ...
  stages-source.subscribe do
    ((new-stages) ->
      razor = (x) ->
        if x.path
          if (last x.path) in <[unread updated-at]>
            return false
        true
