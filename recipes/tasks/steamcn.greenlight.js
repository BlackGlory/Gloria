function extract($) {
  return $('#threadlisttableid [id^=normalthread] a.s.xst').map((i, el) => {
    return {
      message: $(el).text()
    , url: 'http://steamcn.com/' + $(el).attr('href')
    , iconUrl: 'data:image/png;base64,'
    }
  }).get()
}

Promise.all([
  importScripts('gloria-utils')
, fetch('http://steamcn.com/forum.php?mod=forumdisplay&fid=251&filter=author&orderby=dateline').then(res => res.arrayBuffer())
, fetch('http://steamcn.com/forum.php?mod=forumdisplay&fid=161&filter=author&orderby=dateline').then(res => res.arrayBuffer())
])
.then(([{ cheerio }, html1, html2]) => {
  html1 = new TextDecoder('gbk').decode(html1)
  html2 = new TextDecoder('gbk').decode(html2)

  return [
    ...extract(cheerio.load(html1))
  , ...extract(cheerio.load(html2))
  ].filter(({ message: x }) => x.includes('绿光') || x.includes('青睐之光') || x.includes('Greenlight'))
})
.then(commit)
